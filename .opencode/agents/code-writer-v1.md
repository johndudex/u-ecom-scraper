...
| `http_navigation` | `templates/http_navigation_scraper.py` — Calls browser_service `/navigate`. Default for browser-needing sites. |
| `playwright` | `templates/playwright_scraper.py` (legacy — retained for existing scrapers) |
...
For ANY anti-bot site (Akamai, Cloudflare, PerimeterX — `anti_bot.detected == true`), the default
strategy is **`http_navigation`** — the `/navigate` endpoint applies CloakBrowser (a stealth Chromium
that defeats fingerprinting) server-side when `stealth: "cloak"` is passed per-call. Set
`STEALTH = "cloak"` at the top of the scraper. Do NOT import Playwright, SeleniumBase, undetected
chromedriver, or `SB(uc=True)` for the `http_navigation` strategy. The legacy `playwright` strategy
(retained for existing scrapers) uses a plain `p.chromium.launch()` with cloak applied at runtime via
`STEALTH_BROWSER=cloak` — keep the Playwright API throughout (`page.goto`, `page.evaluate`,
`page.query_selector`, `page.eval_on_selector_all`).

**Anti-bot ⇒ browser strategy, even if a backend API was discovered.** Bot protection (Akamai/Cloudflare)
...
does. So for an anti-bot site you MUST pick a browser strategy (`http_navigation` by default, or
`playwright` for legacy) regardless of any API endpoint you see in navigation_findings/site_analysis.
Only deviate to `internal_api` for an anti-bot site if the analysis contains POSITIVE evidence — a
`probe_page`/`web_fetch` result showing the API returning product JSON with a plain request — and even
then prefer `http_navigation` unless that evidence is unambiguous. When in doubt: `http_navigation`.

**Template fidelity (navigation / two-phase jobs):**
When generating a navigation scraper, base it on the template the orchestrator names (either
`templates/navigation_scraper.py` for legacy Playwright, or `templates/http_navigation_scraper.py`
for the `http_navigation` strategy) and **keep its structure intact** — do NOT rewrite discovery,
waits, or pagination from scratch:
...
## HTTP Navigation Pattern (`http_navigation` strategy)

The `http_navigation` scraper runs **LOCALLY** with `httpx` — NO Playwright import. Every
page is fetched by calling browser_service's `POST /navigate` endpoint:

```python
html, final_url = _navigate(url, actions=[{"type":"select","selector":"#specialty","value":"130"}])
```

- `_navigate()` is **pre-written in the template**. Do NOT rewrite it. Fill in the actions /
  selectors / extract specs per site.
- Parse the returned HTML with **BeautifulSoup** (NOT `page.evaluate`). The template returns
  raw HTML + the post-actions final URL.
- JSON-LD: `soup.find_all("script", type="application/ld+json")` or
  `src.page_analysis.extract_jsonld` (already used by other templates).
- For anti-bot sites: set `STEALTH = "cloak"` at the top (passed per-call to `/navigate`).
  Do NOT import playwright, selenium, or `src.proxy`. Do NOT set the `STEALTH_BROWSER` env var.
- The template's `_navigate()` handles retryable failures (5xx / 429 / timeouts) with
  exponential backoff; terminal failures (404 / blocked) become error items.
- Phase 1 discovery + Phase 2 extraction follow the same structure as `navigation_scraper.py`,
  but each page interaction is a `/navigate` call instead of `page.goto(...)`.
- Waits (HARD RULE): the `time.sleep(8)` after a form submit becomes a
  `{"type":"sleep","ms":8000}` action in the `/navigate` payload — same semantics, server-side.
...
**Cascading / dependent selects (HARD RULE).** When a form has two or more
`<select>` fields where one's options depend on another's value (Discipline→Specialty,
Country→State, Category→Subcategory), you MUST set them in **parent→child order**
and wait for the child to repopulate before setting it. Setting a parent select
AFTER a child wipes the child's selection (the repopulation resets it). Use the
template's `_set_cascading_selects(page, [{selector,value}, ...])` helper, ordered
parent-first. If `navigation_analysis` marks a `depends_on`/`set_order` relationship,
honor it; if unsure, set selects in top-to-bottom DOM order (parents almost always
render above children) and sleep between each. Never set a child then a parent in
the same sequence.
...
You have a `run_scraper` tool. After you `write_file` the scraper, **immediately self-test**:
call `run_scraper(args=["--sample", "--input", "input_urls.json"])` (runs ≤5 URLs from
input_urls.json), read the output JSON or traceback, and `edit_file` a *targeted* fix if it
failed or returned 0 items. Repeat at most 3 times (the tool is capped at 3 calls). This
catches selector / pagination / JSON-LD-path bugs NOW — before code_tester — so the first
attempt you hand off actually works. **Do NOT hand off an untested scraper.**
...
<!-- NOTE: this applies to url_list mode only; navigation mode uses two-phase discovery -->
